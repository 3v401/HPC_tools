...
Packages that are not common between modules are not shared since they are not of interest for the 'interdependency checker'.
"""
...
print("#####################################\n")
print("Welcome to the 'Interdependency checker'. This program checks all the modules that share the same Package. Packages that are not shared between different module won't be displayed.")
print("")
print("#####################################\n")
...
instruction = input("Press 1 if you want to see the modules that share same package with DIFERENT version. Press 2 if you want to see the modules that share the same package with SAME version: ")
...
if instruction == '1':
    print("You selected: modules that share same package with DIFFERENT version.")
    ...
    print("You selected: modules that share the same package with SAME version.")
    for package, versions in comparison_dict.items():
        if len(set(versions.values())) == 1 and len(set(versions.keys())) != 1:
            # If the number of duplicates is > 1 in version.values() -> Versions are not the same
            # If the number of duplicates is != 1 in version.keys() -> There is more than one module that uses this package
